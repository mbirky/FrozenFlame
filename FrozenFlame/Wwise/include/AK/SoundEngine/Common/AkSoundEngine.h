...
namespace AK
{
	...
	namespace SoundEngine
	{
		...
		/// \sa 
		/// - AkSpeakerConfig.h
		AK_EXTERNAPIFUNC( AkPanningRule, GetPanningRule )();
		
		/// Allows the game to set the panning rule.
		/// This may be changed anytime once the sound engine is initialized.
		/// Note: Some platforms only support one panning rule.
		/// \return 
		/// - AK_NotImplemented if not supported on the platform.
		/// - AK_Success if successful
		AK_EXTERNAPIFUNC( AKRESULT, SetPanningRule )( 
			AkPanningRule in_panningRule ///< Panning rule.
			);
		...
		/// - AK::SoundEngine::SetAuxBusVolumes
		...
		/// Set the Auxilliary busses to route the specified game object
		/// The array size cannot exceed AK_MAX_AUX_PER_OBJ.
		/// To clear the game object's auxilliary sends, in_uNumSendValues must be 0.
		...
		/// This function do nothing on the Wii platform.
		...
		/// - AK::SoundEngine::GetIDFromString()
		/// \return Always returns AK_Success
		AK_EXTERNAPIFUNC( AKRESULT, SetAuxBusVolumes )( 
			const AkAuxBusID in_AuxBusID,			///< Aux bus ID
		...
		///			In the release configuration, this function returns AK_NotCompatible.
		/// \remark 
