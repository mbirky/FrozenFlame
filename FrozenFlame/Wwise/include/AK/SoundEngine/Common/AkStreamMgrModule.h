...
struct AkDeviceSettings
{
	...
	AkReal32			fMaxCacheRatio;				///< If greater than 1, the device attempts to reuse IO buffers that have already been streamed from disk. This is particularly useful when streaming small looping sounds. The drawback is a small CPU hit when allocating memory, and a slightly larger memory footprint in the StreamManager pool. 
													///< The ratio value determines how many structures referencing IO buffers are pre-allocated (from the Stream Manager's memory pool, AkStreamMgrSettings::uMemorySize). For example, if uMaxCacheRatio is 2, the total amount of allocated references is NUM_BUFFERS * 2. 
													///< The column "Caching Available" of the Streaming Devices tab of the Wwise Profiler is a direct illustration of this. It illustrates (fMaxCacheRatio-1) * NUM_BUFFERS. When it reaches 0 (empty), no more additional referencing may occur. This means that although there may already be data usable 
													///< by a stream in the IO pool, the stream cannot use it.
};
