...
struct AkSpeakerVolumeMatrixCallbackInfo : public AkEventCallbackInfo
{
	...
	AkSpeakerVolumes * pVolumesWet[ AK_VOICE_MAX_NUM_CHANNELS ]; ///< For each channel in the sound, pointer to AkSpeakerVolume describing contribution to destination channels (wet mix). Only present if bUseWetVolumes is true
	AkUInt32 uChannelMask;			///< Speaker setup of the sound (for example, AK_SPEAKER_SETUP_STEREO)
	bool bUseWetVolumes;			///< True if the sound can be feeding an auxilliary bus.
};
