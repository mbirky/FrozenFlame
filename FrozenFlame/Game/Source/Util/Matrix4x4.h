...
//	bool calcProjectionMatrixDeg(float fFOV_degrees, float fAspect, float Znear, float Zfar);
   ...
   friend matrix4<T> interpolateMatrix( const matrix4<T> &MatrixA, const matrix4<T> &MatrixB, double lamda)
   {
		...
		else
		{
			...
			{
				T S = sqrt( 1.0 + iMat.ma[0] - iMat.ma[5] - iMat.ma[10] ) * 2;
				quatResult[0] = 0.25 * S;
			...
			{
				T S = sqrt( 1.0 + iMat.ma[5] - iMat.ma[0] - iMat.ma[10] ) * 2;
			...
			{
				T S = sqrt( 1.0 + iMat.ma[10] - iMat.ma[0] - iMat.ma[5] ) * 2;
